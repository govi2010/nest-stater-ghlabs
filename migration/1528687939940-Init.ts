...
export class Init1528687939940 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<any> {
        await queryRunner.query("CREATE TABLE `content_type` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `title` varchar(255) NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `permission` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `title` varchar(255) NOT NULL, `content_type_id` int NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `group` (`id` int NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `title` varchar(255) NOT NULL, UNIQUE INDEX `IDX_8a45300fd825918f3b40195fbd` (`name`), UNIQUE INDEX `IDX_326ae60c2267f5780f1ecc09fa` (`title`), PRIMARY KEY (`id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `user` (`id` int NOT NULL AUTO_INCREMENT, `password` varchar(128) NOT NULL, `last_login` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6), `is_superuser` tinyint NOT NULL DEFAULT 0, `username` varchar(150) NOT NULL, `first_name` varchar(30) NOT NULL, `last_name` varchar(30) NOT NULL, `email` varchar(254) NOT NULL, `is_staff` tinyint NOT NULL DEFAULT 0, `is_active` tinyint NOT NULL DEFAULT 0, `date_joined` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), `date_of_birth` datetime NULL, UNIQUE INDEX `IDX_78a916df40e02a9deb1c4b75ed` (`username`), PRIMARY KEY (`id`)) ENGINE=InnoDB");
